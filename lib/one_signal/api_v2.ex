defmodule OneSignal.API_V2 do
  ...
  def request(body, method, endpoint, headers, opts) do
    ...
    req_url =
      ...
      |> prepend_url("#{base_url}#{app_id}#{endpoint}")

    perform_request(req_url, method, "", headers, opts)
  end
    ...
    error =
      case json_library().decode(body) do
        {:ok, %{"error_description" => _} = api_error} ->
          Error.from_onesignal_error(status, api_error, request_id)
        ...
        {:error, _} ->
          ...
          Error.from_onesignal_error(status, nil, request_id)
      end
